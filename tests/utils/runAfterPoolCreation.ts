 ...
 */
import { web3, BN, Provider, setProvider, Wallet } from "@project-serum/anchor";
import {
  PublicKey,
  SignerWallet,
...
import { setupEscrowAndLockTokens } from "./setupEscrowAndLockTokens";
...
import { FeeAmount, POOL_SEED, tradeComparator, u32ToSeed } from "@cykura/sdk";
...
(async function() {
  ...
  t = new web3.Transaction();
  await pro.send(t.add(...createLockerTx.instructions), createLockerTx.signers);
  console.log("THIRD WORKS", locker.toString());

  const lockerWrapper = await LockerWrapper.load(tribecaSdk, locker, governor);
  ...
  const poolState = new web3.PublicKey(
    "DHmTuxkoquwQiWcCq2aHtEKejprEujkwwngYAksXpywg"
  );
  ...
  const startTime = new BN(blockTime + 3600);
  const endTime = new BN(blockTime + 86400);

  const {
    wrapper: _incentiveWrapper,
    tx: createIncentiveTx,
