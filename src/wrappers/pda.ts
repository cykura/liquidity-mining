...
export const findStakeManagerAddress = async (): Promise<
  [PublicKey, number]
> => {
...
export const findDepositAddress = async (
  ...
  return PublicKey.findProgramAddress(
    [utils.bytes.utf8.encode('Deposit'), mint.toBuffer()],
    CYKURA_STAKER_ADDRESSES.CykuraStaker
  );
};
...
export const findBondManager = async (): Promise<[PublicKey, number]> => {
  return PublicKey.findProgramAddress(
    [],
    CYKURA_STAKER_ADDRESSES.BondedCys
  );
};